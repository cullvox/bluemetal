...
void main() {
	...
	vec2 skyUV = material.eyeDirection.xz / material.eyeDirection.y;
	
	float base_n = texture(base_noise, (skyUV - inTime) * material.base_noise_scale).x;
	
	// get the middle -> abs of the material.eyeDirection to get the horizon
	float horizon = abs((material.eyeDirection.y * material.horizon_intensity) - material.offset_horizon);
	...
	// sun creation
	float sun = distance(material.eyeDirection.xyz, material.sunDirection);
	...
	// moon creation
	float moon = distance(material.eyeDirection.xyz, - material.sunDirection);
	float crescentMoon = distance(vec3(material.eyeDirection.x + material.moon_crescent, material.eyeDirection.yz), - material.sunDirection);
	...
	// day color gradient
	vec3 gradientDay = mix(material.day_bottom_color.rgb, material.day_top_color.rgb, clamp(material.eyeDirection.y, 0.0, 1.0));
	// night color gradient
	vec3 gradientNight = mix(material.night_bottom_color.rgb, material.night_top_color.rgb, clamp(material.eyeDirection.y, 0.0, 1.0));
