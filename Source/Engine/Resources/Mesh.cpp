...
{
    _vertexBuffer = VulkanBuffer{_system->GetDevice(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, bytes.size(), nullptr, false};
    _vertexBuffer.Upload(bytes);
}

void Mesh::UploadIndices(std::span<uint32_t> indices)
{
    _indexBuffer = VulkanBuffer{_system->GetDevice(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, indices.size_bytes(), nullptr, false};
