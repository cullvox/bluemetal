...
public:
    blRenderer(std::shared_ptr<blRenderDevice> renderDevice, std::shared_ptr<blSwapchain> swapchain, std::vector<std::shared_ptr<blRenderPass>>& passes);
    ~blRenderer();

    void resize(blExtent2D extent);
    void submit(const blSubmission& submission) noexcept;
