...
void InspectorEditor::Draw(RenderData& rd)
{
    ...
    std::string_view className = selectedNode->GetClassName();
    while (!className.empty()) {
        ...
        for (const auto& prop : properties) {
            ...
            auto var = prop->Get(selectedNode);
            std::visit([&](auto&& value) {
                ...
                if constexpr (std::is_same_v<T, EnumValue>) {
                    ...
                    if (ImGui::BeginCombo(("##" + std::string(prop->GetName())).c_str(), current.data())) {
                        for (int i = 0; i < enumValues.size(); i++) {
                            ...
                            if (ImGui::Selectable(enumValues[i].first.data(), isSelected)) {
                                ...
                                prop->Set(selectedNode, EnumValue{value.enumName, enumValues[i].second});
                            }
                    ...
                    ImGui::DragFloat(("##" + std::string(prop->GetName())).c_str(), &value);
                    prop->Set(selectedNode, value);
                    ...
                    if (ImGui::InputText(("##" + std::string(prop->GetName())).c_str(), buffer, sizeof(buffer))) {
                        prop->Set(selectedNode, std::string(buffer));
                    }
                } else if constexpr (std::is_same_v<T, glm::vec3>) {
                    ImGui::DragFloat3(("##" + std::string(prop->GetName())).c_str(), &value.x, 0.1f);
                    prop->Set(selectedNode, value);
                } else if constexpr (std::is_same_v<T, glm::quat>) {
                    glm::vec3 euler = glm::degrees(glm::eulerAngles(value));
                    ImGui::DragFloat3(("##" + std::string(prop->GetName())).c_str(), &euler.x, 0.1f);
                    value = glm::quat(glm::radians(euler));
                    prop->Set(selectedNode, value);
                } else if constexpr (std::is_same_v<T, glm::vec2>) {
                    ImGui::DragFloat2(("##" + std::string(prop->GetName())).c_str(), &value.x, 0.1f);
                    prop->Set(selectedNode, value);
                } else {
